...
| `scripts/staging_infrastructure_validation.py` | bounded strict UTF-8 JSON domain과 key-sorted canonical JSON helper |
...
TDD는 각 component에서 RED test를 먼저 추가하고 구현 후 GREEN으로 전환했습니다. 이후 approval validation, action provenance, untrusted evidence, gate binding과 최종 integration review를 반복했습니다. `7f4ece3c`에서 raw bytes 필수화, gate `--plan`, validator completion marker, pre-read size bound, strict Unicode/canonical serialization을 통합 보정했습니다.
...
- 모든 JSON은 read 전 1,000,000-byte 제한과 strict UTF-8/JSON-domain 제한을 적용합니다. exact raw digest와 sorted canonical object digest를 별도로 검증하며 raw bytes 없이는 action/gate provenance가 성립하지 않습니다.
...
| `python3 -m unittest discover -s scripts/tests -p 'test_*.py' -v` | `Ran 146 tests` / `OK` (0.794초) |
...
| production line count | 436, 496, 433, 52, 56줄로 모두 500줄 미만입니다. |
