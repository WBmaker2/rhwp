...
83d88e77276bf8a5734dc1c204432b2f1bfef1a9 docs: document bounded staging evidence reader
...
| `scripts/staging_infrastructure_validation.py` | shared bounded raw-byte API, `O_NONBLOCK | O_CLOEXEC` same-FD `fstat` regular-file reader와 special-file rejection, strict UTF-8 JSON domain 및 key-sorted canonical JSON helper |
...
TDD는 각 component에서 RED test를 먼저 추가하고 구현 후 GREEN으로 전환했습니다. 이후 approval validation, action provenance, untrusted evidence, gate binding과 최종 integration review를 반복했습니다. `7f4ece3c`에서 raw bytes 필수화를, `8f63836b`에서 public API byte limit과 bounded reader를, `426b71ac`에서 nonblocking FIFO/special-file rejection을 공통 validation 계층에 통합했습니다.
...
- 모든 JSON file 입력은 `O_NONBLOCK | O_CLOEXEC`로 열어 같은 FD의 `fstat`로 regular file을 확인하므로 FIFO/special file을 차단 없이 거부합니다. 이후 최대 `MAX_JSON_BYTES + 1`만 읽고 public raw-byte API에도 1,000,000-byte 제한을 적용합니다.
...
| `python3 -m unittest discover -s scripts/tests -p 'test_*.py' -v` | `Ran 151 tests` / `OK` (0.895초) |
...
| production line count | 436, 493, 434, 52, 101줄로 모두 500줄 미만입니다. |
